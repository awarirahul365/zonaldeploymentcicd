...
async def http_trigger_zonal(req: func.HttpRequest) -> func.HttpResponse:
    ...
    obj = BlobService(
        storageaccount_endpoint=os.getenv("Storageaccount_endpoint"),
        conn_str=os.getenv("conn_str"),
    )
    result = await obj.read_container_file(
        blob_name=os.getenv("ded_blob_name"),
        container_name=os.getenv("dedcontainername"),
    )
    ...
    updated_ci_ai_list = [
        elem for elem in ci_ai_list if elem[0]["SIDCID"] in ded_list_filtered
    ]
    ...
    if dfzonal_ci_ai.empty is not True:
        df_merge_zonal_ci_ai = pd.merge(
            df_ded_list, dfzonal_ci_ai, on="SIDCID", how="inner"
        )
    ...
    with pd.ExcelWriter(result_xlsx, engine="openpyxl") as writer:
        ...
        if df_merge_nonzonal is not None:
            df_merge_nonzonal.to_excel(
                writer, sheet_name="Non Zonal System", index=False
            )

    result_xlsx.seek(0)

    output = await obj.upload_blob_to_container(
        data=result_xlsx,
        file_name=os.getenv("zonal_result_file_name"),
        container_name=os.getenv("resultcontainer"),
    )
    print(output)
